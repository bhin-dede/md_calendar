...
2. src/lib/db.ts - 데이터 모델 및 API
3. src/lib/types.ts - TypeScript 타입 정의
4. src/app/layout.tsx - 앱 구조
...
4. src/styles/components.css에 스타일 추가
...
2. src/lib/db.ts의 DB_VERSION 증가
3. onupgradeneeded에서 마이그레이션 로직 추가
4. 새 인덱스나 필드 정의
5. 관련 CRUD 함수 추가/수정
```
...
| hydration 에러 | 'use client' 디렉티브 확인 |
| IndexedDB 에러 | 브라우저 DevTools > Application > IndexedDB 확인 |
| 스타일 미적용 | CSS import 순서 및 캐시 확인 (Ctrl+Shift+R) |
| DOMPurify 에러 | dynamic import 사용 확인 |
